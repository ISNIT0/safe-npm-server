...
const router_1 = __webpack_require__(4);
const db_1 = __webpack_require__(23);
const app = express();
app.use(logger('tiny'));
...
const { statusRouter } = __webpack_require__(5);
const router = Router();
...
const Repository = __webpack_require__(6);
const asyncHandler = __webpack_require__(12);
const statusReport_model_1 = __webpack_require__(13);
...
router.get('/:packageName/:version.:format(png|json)', asyncHandler((req, res) => __awaiter(this, void 0, void 0, function* () {
    const { packageName, version, format } = req.params;
    const packageStatus = yield statusReport_model_1.StatusReport.findOne({ packageName, version });
    if (packageStatus) {
        if (!!~pendingStates.indexOf(packageStatus.automaticTestStatus)) {
            const build = yield getTravisBuild(packageName, version);
            const buildStatus = build.branch.state;
            packageStatus.automaticTestStatus = buildStatus;
            const savedStatus = yield packageStatus.save();
            if (format === 'json') {
        ...
        const status = new statusReport_model_1.StatusReport();
        status.packageName = packageName;
        status.version = version;
        status.automaticTestStatus = 'created';
        yield startPackageTest(packageName, version);
        const savedStatus = yield status.save();
        if (format === 'json') {
            res.json(savedStatus);
        }
        else {
...
const repo = new Repository('ISNIT0/npm-package-tester', {
    token: config_1.config.github.token
});
function startPackageTest(packageName, version) {
    return __awaiter(this, void 0, void 0, function* () {
        const newBranch = `auto/${packageName}_${version}`;
        const newContent = JSON.stringify({ packageName, version }, null, '\t');
        yield repo.createBranch('master', newBranch);
        try {
            yield repo.writeFile(newBranch, 'test.json', newContent, `Automatically triggering test for [${packageName}@${version}]`, {});
        }
        catch (err) {
            console.error(err);
        }
...
function getTravisBuild(packageName, version) {
    return axios_1.default.get(`https://api.travis-ci.org/repos/ISNIT0/npm-package-tester/branches/auto/${packageName}_${version}`, {
        headers: {
            Authorization: `token "${config_1.config.travis.token}"`
        }
    }).then(a => a.data);
}
...
var _Requestable2 = __webpack_require__(7);
...
let StatusReport = class StatusReport extends typeorm_1.BaseEntity {
};
__decorate([
    typeorm_1.PrimaryGeneratedColumn('uuid'),
    __metadata("design:type", String)
], StatusReport.prototype, "id", void 0);
__decorate([
    typeorm_1.Column(),
    __metadata("design:type", String)
], StatusReport.prototype, "packageName", void 0);
__decorate([
    typeorm_1.Column(),
    __metadata("design:type", String)
], StatusReport.prototype, "version", void 0);
__decorate([
    typeorm_1.Column(),
    __metadata("design:type", String)
], StatusReport.prototype, "automaticTestStatus", void 0);
__decorate([
    typeorm_1.Column({ nullable: true }),
    __metadata("design:type", Date)
], StatusReport.prototype, "automaticTestCompletedAt", void 0);
StatusReport = __decorate([
    typeorm_1.Entity()
], StatusReport);
...
const convict = __webpack_require__(21);
var Env;
(function (Env) {
    Env["Test"] = "test";
...
(function () {
  ...
  __webpack_require__(18).config(options)
})()
...
const connectOptions = () => ({
    ...
    logging: false,
    entities: [
        statusReport_model_1.StatusReport
    ],
    cli: {
...
const postgresConfig = convict({
    ...
    dbName: {
        doc: 'Postgres default database name',
        format: String,
        default: 'safenpm-test',
        env: 'DATABASE_NAME',
    },
